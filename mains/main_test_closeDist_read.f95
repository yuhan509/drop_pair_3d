...
!!//  rotate the grid so that the pole coincides with the closest point to origin
      ...
      allocate(beta(nbeta))
      allocate(xr1grids(nphi,ntheta,nrot,nbeta),yr1grids(nphi,ntheta,nrot,nbeta), &
      zr1grids(nphi,ntheta,nrot,nbeta))
      allocate(xr2grids(nphi,ntheta,nrot,nbeta),yr2grids(nphi,ntheta,nrot,nbeta), &
      zr2grids(nphi,ntheta,nrot,nbeta))
      allocate(rx1mpoles(0:nterms,0:nterms,nrot,nbeta), ry1mpoles(0:nterms,0:nterms,nrot,nbeta), &
      rz1mpoles(0:nterms,0:nterms,nrot,nbeta))
      allocate(rx2mpoles(0:nterms,0:nterms,nrot,nbeta), ry2mpoles(0:nterms,0:nterms,nrot,nbeta), &
      rz2mpoles(0:nterms,0:nterms,nrot,nbeta))

      beta(1) = theta_cl
      call rotgrid_fsr_real_init(nterms,nbeta,beta,rotmat_one)

      call rotgrid_fsr_real_mpolesout(nterms,x1mpole,nphi,ntheta,  &
        nbeta,beta,nrot,xr1grids,rotmat_one,ctheta,ynms,dwsave,rx1mpoles)
      call rotgrid_fsr_real_mpolesout(nterms,y1mpole,nphi,ntheta,  &
        nbeta,beta,nrot,yr1grids,rotmat_one,ctheta,ynms,dwsave,ry1mpoles)
      call rotgrid_fsr_real_mpolesout(nterms,z1mpole,nphi,ntheta,  &
        nbeta,beta,nrot,zr1grids,rotmat_one,ctheta,ynms,dwsave,rz1mpoles)
        
      k = nphi/2 + 1
      do i = 1, ntheta
        do j = 1, nphi
          x1grid(j,i) = xr1grids(j,i,k,1)
          y1grid(j,i) = yr1grids(j,i,k,1)
          z1grid(j,i) = zr1grids(j,i,k,1)
        enddo
      enddo

      do n = 0, nterms
        do m = 0, n
          x1mpole(n,m) = rx1mpoles(n,m,k,1)
          y1mpole(n,m) = ry1mpoles(n,m,k,1)
          z1mpole(n,m) = rz1mpoles(n,m,k,1)
        enddo 
      enddo  
      ...
      call rotgrid_fsr_real_mpolesout(nterms,x2mpole,nphi,ntheta,  &
        nbeta,beta,nrot,xr2grids,rotmat_one,ctheta,ynms,dwsave,rx2mpoles)
      call rotgrid_fsr_real_mpolesout(nterms,y2mpole,nphi,ntheta,  &
        nbeta,beta,nrot,yr2grids,rotmat_one,ctheta,ynms,dwsave,ry2mpoles)
      call rotgrid_fsr_real_mpolesout(nterms,z2mpole,nphi,ntheta,  &
        nbeta,beta,nrot,zr2grids,rotmat_one,ctheta,ynms,dwsave,rz2mpoles)
        
      k = 1
      do i = 1, ntheta
        do j = 1, nphi
          x2grid(j,i) = xr2grids(j,i,k,1)
          y2grid(j,i) = yr2grids(j,i,k,1)
          z2grid(j,i) = zr2grids(j,i,k,1)
        enddo
      enddo

      do n = 0, nterms
        do m = 0, n
          x2mpole(n,m) = rx2mpoles(n,m,k,1)
          y2mpole(n,m) = ry2mpoles(n,m,k,1)
          z2mpole(n,m) = rz2mpoles(n,m,k,1)
        enddo 
      enddo  
