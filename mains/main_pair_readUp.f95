...
!! drop 2
    ...
    real *8 :: START,END
!！ reading
    complex *16, ALLOCATABLE:: x1mpole_pre(:,:), y1mpole_pre(:,:), z1mpole_pre(:,:)  
    complex *16, ALLOCATABLE:: x2mpole_pre(:,:), y2mpole_pre(:,:), z2mpole_pre(:,:)  
...
!!ccccccccccccccccccccccccccccccccccccccccccccccc
    ...
    nterms_pre = 19
    allocate(x1mpole_pre(0:nterms_pre,0:nterms_pre), y1mpole_pre(0:nterms_pre,0:nterms_pre), z1mpole_pre(0:nterms_pre,0:nterms_pre))
    allocate(x2mpole_pre(0:nterms_pre,0:nterms_pre), y2mpole_pre(0:nterms_pre,0:nterms_pre), z2mpole_pre(0:nterms_pre,0:nterms_pre))
    open(15, file = 'fortcopy.162', status='old')
    read(15,*) !! skip reading the first line
    write(*,*) "read intial grid drop1"
    do n = 0, nterms_pre
        do m = 0, n
        read(15,*) na, nb, x1mpole_pre(n,m), y1mpole_pre(n,m), z1mpole_pre(n,m)
        write(*,*) na, nb, x1mpole_pre(n,m), y1mpole_pre(n,m), z1mpole_pre(n,m)
      enddo
    enddo
    
    close(15)
    
    open(15, file = 'fortcopy.262', status='old')
    read(15,*) !! skip reading the first line
    write(*,*) "read intial grid drop2"
    do n = 0, nterms_pre
        do m = 0, n
        read(15,*) na, nb, x2mpole_pre(n,m), y2mpole_pre(n,m), z2mpole_pre(n,m)
        write(*,*) na, nb, x2mpole_pre(n,m), y2mpole_pre(n,m), z2mpole_pre(n,m)
      enddo
    enddo
    
    close(15)      
    
!!!! upsample read to current nterms 
    do n = 0, nterms
        do m = 0, n
            x1mpole(n,m) = (0d0,0d0)
            y1mpole(n,m) = (0d0,0d0)
    ...
    do n = 0, nterms_pre
        do m = 0, n
            x1mpole(n,m) = x1mpole_pre(n,m)
            y1mpole(n,m) = y1mpole_pre(n,m)
            z1mpole(n,m) = z1mpole_pre(n,m)
            x2mpole(n,m) = x2mpole_pre(n,m)
            y2mpole(n,m) = y2mpole_pre(n,m)
            z2mpole(n,m) = z2mpole_pre(n,m)
        enddo 
    enddo
!!!!
...
!!ccccccccccccccccccccccccccccccccccccccccccccccc
!! drop1
    call sphtrans_real(nterms,x1mpole,nphi,ntheta,x1grid, &
         ctheta,ynms,dwsave)
    call sphtrans_real(nterms,y1mpole,nphi,ntheta,y1grid, &
         ctheta,ynms,dwsave)    
    call sphtrans_real(nterms,z1mpole,nphi,ntheta,z1grid, &
         ctheta,ynms,dwsave)    
!! drop 2
    call sphtrans_real(nterms,x2mpole,nphi,ntheta,x2grid, &
         ctheta,ynms,dwsave)
    call sphtrans_real(nterms,y2mpole,nphi,ntheta,y2grid, &
         ctheta,ynms,dwsave)    
    call sphtrans_real(nterms,z2mpole,nphi,ntheta,z2grid, &
         ctheta,ynms,dwsave)    
    ...
    !! equally distributed theta sample gridGerg for En2
