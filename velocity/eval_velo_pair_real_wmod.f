      SUBROUTINE eval_velo_pair_wmod(istep, Ca, iup, alpha,
     $ hintpol, nintpol, h_far,
     $ x1grid, y1grid, z1grid, x1mpole, y1mpole, z1mpole, 
     $ h1mpole, ws1mpole, n1mpole, 
     $ h1grid, ws1grid, n1grid, u1grid, en1mpole,  
     $ x2grid, y2grid, z2grid, x2mpole, y2mpole, z2mpole, 
     $ h2mpole, ws2mpole, n2mpole,
     $ h2grid, ws2grid, n2grid, u2grid, en2mpole)
      ...
      integer :: nintpol
      real *8 :: Ca, hintpol, alpha

      real *8 :: x1grid(nphi,ntheta),y1grid(nphi,ntheta),
     $ z1grid(nphi,ntheta),h1grid(nphi,ntheta),ws1grid(nphi,ntheta), 
     $ n1grid(nphi,ntheta,3)
      complex *16 :: x1mpole(0:nterms,0:nterms),
     $ y1mpole(0:nterms,0:nterms),z1mpole(0:nterms,0:nterms)
      complex *16 :: h1mpole(0:nterms,0:nterms),
     $ ws1mpole(0:nterms,0:nterms), n1mpole(0:nterms,0:nterms,3)

      real *8 :: x2grid(nphi,ntheta),y2grid(nphi,ntheta),
     $ z2grid(nphi,ntheta),h2grid(nphi,ntheta),ws2grid(nphi,ntheta), 
     $ n2grid(nphi,ntheta,3)
      complex *16 :: x2mpole(0:nterms,0:nterms),
     $ y2mpole(0:nterms,0:nterms),z2mpole(0:nterms,0:nterms)
      complex *16 :: h2mpole(0:nterms,0:nterms),
     $ ws2mpole(0:nterms,0:nterms), n2mpole(0:nterms,0:nterms,3)
...
!     $  x2igrid, y2igrid, z2igrid, ws2igrid, h2igrid, n2igrid)
      ...
      do n = 0, nterms
        do m = 0, n
          do i = 1, itheta
            do j = 1, iphi
              sphfigrid(j,i,n,m) = upx_sphfgrid(j,i,n,m)
         ! write(9404,*) upx_sphfgrid(j,i,n,m)
            enddo  
          enddo
        enddo
      enddo
...
cccccc  regular quadrature over drop 2 for source on drop1 
     ...
     $          iterms, itheta, iphi, nintpol, hintpol,
...
!          do i = 1, ntheta
!            do j = 1, nphi
!            cls1d1grids(j,i,n,-m) = dconjg(cls1d1grids(j,i,n,m))
!            cls1d2grids(j,i,n,-m) = dconjg(cls1d2grids(j,i,n,m))
!            cls2d1grids(j,i,n,-m) = dconjg(cls2d1grids(j,i,n,m))
!            cls2d2grids(j,i,n,-m) = dconjg(cls2d2grids(j,i,n,m))
!            enddo
!          enddo 
!        enddo
!      enddo 
...
!          do n1 = 0, nterms
!            do m1 = -n1, n1
!             write(114,*) n1, m1,n, m,cls1d1mpoles(n1,m1,n,m),
!     $  cls1d1mpoles(n1,-m1,n,m), cls1d1mpoles(n1,m1,n,-m),
!     $  cls1d1mpoles(n1,-m1,n,-m)
!                  write(115,*) n1, m1,n, m,cls2d1mpoles(n1,m1,n,m),
!     $  cls2d1mpoles(n1,-m1,n,m), cls2d1mpoles(n1,m1,n,-m),
!     $  cls2d1mpoles(n1,-m1,n,-m)
!            enddo
!          enddo
!        enddo
!      enddo
...
cccccccccccc
  ...
  !   $ " only nonzero A terms, in A * X = B"  
      ! write(201,*)  "istep=",istep, 
  !   $ " only nonzero imaginary of A terms, in A * X = B"  
       do k = 1, N2
         do l = 1, N2
...
!     $ IPIV,cap_Einf,N2-1,INFO)      
      write(116,*) "istep=",istep, 
     $ 'linear solver zgesv solution:','INFO=',INFO
      write(216,*) "istep=",istep, 
     $ 'linear solver zgesv solution:','INFO=',INFO
      i = 1
      do n = 1, nterms !! start 0
          do m = -n, n  !!! real, change to 0   
     ...
     $          quadr1, quadr2, quadr3)
          ...
          do i = 1, ntheta
            do j = 1, nphi
              ...
              enddo
          write(134,*) i,j, u1grid(j,i,1), u1grid(j,i,2), u1grid(j,i,3)
          write(234,*) i,j, u2grid(j,i,1), u2grid(j,i,2), u2grid(j,i,3)
            enddo
          enddo
